"""
src/lead_generation/llm/requirement_extractor.py
────────────────────────────────────────────────
...
from src.shared.models import SearchCriteria
...
def _extract_product(text: str) -> str:
    m = re.search(r"\b(?:specialize in|sell|supply|produce|make|offer[s]?|provide[s]?)\s+([a-z][\w\s-]{2,50}?)(?:\s+(?:and|to|for|with|that|in)\b|[.,])", text, re.I)
    return m.group(1).strip() if m else ""


def _heuristic(text: str) -> SearchCriteria:
    targets: list[str] = []
    for word in ("cafe", "cafes", "restaurant", "restaurants", "hotel", "hotels",
                 "bakery", "bakeries", "gym", "gyms", "fitness studio", "boutique",
                 "boutiques", "clinic", "clinics", "office", "offices", "school",
                 "schools", "coaching center", "coaching centers", "store", "stores",
                 "startup", "startups", "lifestyle store"):
        if re.search(rf"\b{word}\b", text, re.I):
            base = word.rstrip("s")
            if base not in targets:
                targets.append(base)
    name_match = re.search(r"\b(?:i am|we are|this is)\s+([A-Z][\w&' ]{2,40}?)(?:[.,]|\s+(?:is|makes|produces|offers|specializes|based|in|a))", text)
