"""
src/lead_generation/scoring/scorer.py
─────────────────────────────────────
Score leads 0-100 against the company's criteria.

To stay friendly with the Gemini free-tier rate limit, ALL leads are scored in
a single batched call. If that call is unavailable (no key / rate limited),
every lead falls back to a deterministic heuristic — with NO extra API calls.

Gemini judges: relevance, buying potential, location fit, industry fit,
business-size estimate, overall recommendation → {score, reason, gaps, strengths}.
"""
...
_SINGLE_PROMPT = """Score this B2B lead from 0-100 for how well it fits the seller.
...
Return JSON only:
{{"score": <0-100>, "priority": "high|medium|low", "reason": "<one sentence>",
  "strengths": ["<fit signal>", ...], "gaps": ["<missing fit or data>", ...]}}"""
...
def _lead_fields(lead: Lead) -> dict:
    return {k: v for k, v in {
        "name": lead.name, "category": lead.category, "address": lead.address,
...
def _criteria_factors(lead: Lead, criteria: SearchCriteria) -> tuple[list[dict], list[str], list[str], int]:
    factors: list[dict] = []
    matched: list[str] = []
    gaps: list[str] = []
    points = 0
    ...
    for target in criteria.targets:
        ok = target.lower() in blob or (lead.category and target.lower() in lead.category.lower())
    ...
    attr_matches = [a for a in criteria.attributes if a.lower() in blob]
    attr_gaps = [a for a in criteria.attributes if a.lower() not in blob]
...
def _build_breakdown(
    ...
    *,
    ...
    return {
        ...
        "missing": missing_all,
        "present": strengths,
        "attribute_matches": [a for a in criteria.attributes if a.lower() in _lead_blob(lead)],
        "attribute_gaps": [a for a in criteria.attributes if a.lower() not in _lead_blob(lead)],
        "strengths": strengths,
        "gaps": missing_all,
...
def score_lead(lead: Lead, criteria: SearchCriteria) -> Lead:
    """Score a single lead (one Gemini call, heuristic fallback)."""
    result = generate_json(_SINGLE_PROMPT.format(
        product=criteria.product or "(unspecified)",
        targets=", ".join(criteria.targets) or "(any)",
        location=criteria.location or "(any)",
        attributes=", ".join(criteria.attributes) or "(none)",
        lead=json.dumps(_lead_fields(lead), indent=2)[:2000],
    ))
    if result and isinstance(result, dict) and "score" in result:
        return _apply_ai(lead, result, criteria)
    return _apply_heuristic(lead, criteria)


def score_leads(leads: list[Lead], criteria: SearchCriteria) -> list[Lead]:
    """Score every lead in ONE batched Gemini call (heuristic fallback for all)."""
    if not leads:
        return []

    items = [{"i": idx, **_lead_fields(lead)} for idx, lead in enumerate(leads)]
    result = generate_json(_BATCH_PROMPT.format(
        product=criteria.product or "(unspecified)",
        what_they_do=criteria.what_they_do or "(unspecified)",
        targets=", ".join(criteria.targets) or "(any)",
        location=criteria.location or "(any)",
        attributes=", ".join(criteria.attributes) or "(none)",
        extra=criteria.additional_requirements or "(none)",
