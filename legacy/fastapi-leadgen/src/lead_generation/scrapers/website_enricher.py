"""
Fetch business websites and extract contact info from HTML.
...
from src.lead_generation.scrapers import site_analyzer
from src.shared.config import config
from src.shared.utils.phone import clean_phone
...
def _fetch_html(url: str) -> str:
    ...
    with urllib.request.urlopen(req, timeout=config.serpapi_website_timeout) as resp:
        raw = resp.read(_MAX_BYTES)
        ctype = resp.headers.get("Content-Type", "").lower()
        if raw and ("html" in ctype or "text" in ctype or "<" in raw[:200].decode(errors="ignore")):
...
def _candidate_urls(start_url: str, homepage_html: str, *, max_pages: int = _MAX_PAGES_SINGLE) -> list[str]:
    ...
    for href in _CONTACT_HREF_RE.findall(homepage_html):
        ...
        if url and _same_site(url, base) and url.rstrip("/") not in seen:
            seen.add(url.rstrip("/"))
            urls.append(url)
...
def _merge_enrichment(row: dict, pages: list[tuple[str, str]], site_domain: str | None, *, site_meta: dict | None = None) -> dict:
...
def enrich_expand(row: dict) -> list[dict]:
    ...
    merged = _enrich_single({**row, "metadata": {**(row.get("metadata") or {}), "site_type": site_type, "classification": classification}}, deep=True)
...
async def enrich_all(candidates: list[dict], *, on_progress=None) -> list[dict]:
    sem = asyncio.Semaphore(config.serpapi_website_concurrency)
