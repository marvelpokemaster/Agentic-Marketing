...
def _check_rate_limit() -> bool:
    ...
    return len(_recent_calls) < config.serpapi_rate_limit_per_min
...
def _log_raw(query: str, start: int, data: dict) -> None:
    if not config.serpapi_log_raw:
        return
    log_dir = os.path.join(config.outputs_dir, "serpapi_logs")
...
def fetch_page(query: str, start: int = 0) -> dict:
    ...
    url = f"{config.serpapi_base_url}?{urllib.parse.urlencode(params)}"
    last_err = None
    for attempt in range(_MAX_RETRIES):
        if not _check_rate_limit():
            raise RuntimeError(
                f"SerpApi rate limit exceeded ({config.serpapi_rate_limit_per_min}/min)"
            )
        try:
...
def fetch_all(query: str, max_results: int, *, on_page=None) -> list[dict]:
    ...
    organic_count = 0
    start = 0
    for _ in range(config.serpapi_max_pages):
