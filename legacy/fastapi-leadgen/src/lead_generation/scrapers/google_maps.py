...
from src.lead_generation.scrapers.base import PlaywrightScraper, register_scraper
from src.lead_generation.scrapers import website_enricher
...
class GoogleMapsScraper(PlaywrightScraper):
    ...
    async def scrape_target(self, page, target, location, max_results) -> list[dict]:
        ...
        for i in range(cap):
            try:
                ...
                if not detail.get("name") or detail["name"].lower() in ("results", "search results"):
    ...
    async def _detail(self, page) -> dict:
        ...
        detail: dict = {"source": "google_maps", "scraped_at": datetime.utcnow().isoformat()}
        ...
        rating = await grab(
            lambda: page.get_by_role("img", name=re.compile(r"star", re.I)).first.get_attribute("aria-label")
        )
        detail["rating"] = self.clean_text(rating)
        addr = await grab(
            lambda: page.get_by_role("button", name=re.compile(r"address", re.I)).first.get_attribute("aria-label")
        )
        detail["address"] = self.clean_text(addr.split(":", 1)[-1]) if addr else None
        phone = await grab(
            lambda: page.get_by_role("button", name=re.compile(r"phone", re.I)).first.get_attribute("aria-label")
        )
        detail["phone"] = self.extract_phone(phone) if phone else None
        detail["open_status"] = self.clean_text(
            await grab(lambda: page.get_by_text(re.compile(r"^(Open|Closed|Opens|Closes)", re.I)).first.inner_text())
        )
        detail["website"] = await grab(
            lambda: page.get_by_role("link", name=re.compile(r"website", re.I)).first.get_attribute("href")
        )
        return detail
