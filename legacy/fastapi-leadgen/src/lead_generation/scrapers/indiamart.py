...
class IndiaMartScraper(PlaywrightScraper):
    ...
    async def scrape_target(self, page, target, location, max_results) -> list[dict]:
        ...
        for i in range(cap):
            try:
                card = cards[i]
                name = self.clean_text(await self._text(card, [".companyname", ".company-name", "h2", "h3"]))
                if not name:
                    continue
                ...
                rows.append({
                    "name": name,
                    "address": self.clean_text(await self._text(card, [".cityname", ".city", ".loc"])) or location,
                    "category": self.clean_text(await self._text(card, [".prd-cat", ".cat-name"])) or target,
